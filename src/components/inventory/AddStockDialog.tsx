/**
 * AddStockDialog — Simple dialog for adding finished goods stock.
 *
...
import { useProductInventory } from "@/hooks/convex/useProductInventory";
...
export function AddStockDialog({
  ...
  const [selectedProductId, setSelectedProductId] = useState<Id<"menuProducts"> | "">(
    preselectedProductId ?? ""
  );
  const [quantity, setQuantity] = useState("");
  const [selectedLocationId, setSelectedLocationId] = useState<Id<"storageLocations"> | "">(
    defaultLocationId ?? ""
  );
  ...
  useEffect(() => {
    if (open) {
      ...
      // Default to provided defaultLocationId, or first default location, or first location
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-5 py-2">
          ...
          {preselectedProductId && preselectedProductName ? (
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => setSelectedProductId(v as Id<"menuProducts">)}
              >
          ...
          {/* Quantity */}
          <div className="space-y-2">
            <Label htmlFor="quantity">How many boxes?</Label>
            <Input
              id="quantity"
              type="number"
              min="1"
          ...
          {/* Location Selector */}
          <div className="space-y-2">
            <Label htmlFor="location">Add to location</Label>
            <Select
              value={selectedLocationId}
              onValueChange={(v) => setSelectedLocationId(v as Id<"storageLocations">)}
            >
