...
**"phase" or "milestone":** First verify starting point, then create branch:

1. **CRITICAL: Verify you're on main before branching.**
   ```bash
   CURRENT_BRANCH=$(git branch --show-current)
   ```
   If `CURRENT_BRANCH` is NOT `main`: STOP. Warn the user: "Current branch is `{CURRENT_BRANCH}`, not `main`. Branching from a feature branch will interleave commits. Switch to main first: `git switch main && git pull`." Ask user to confirm before proceeding.
