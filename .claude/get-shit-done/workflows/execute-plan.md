...
@D:/Claude/Product Manager/product_master/.claude/get-shit-done/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE}")
...
```bash
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" phases list --type summaries --raw
...
See `D:/Claude/Product Manager/product_master/.claude/get-shit-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
node D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See D:/Claude/Product Manager/product_master/.claude/get-shit-done/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `D:/Claude/Product Manager/product_master/.claude/get-shit-done/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="run_seed_functions">
**Run seed functions after plan execution (idempotent upserts — safe after every plan, non-blocking on failure).**
...
**If `SEED_FUNCTIONS` is empty or missing:** Skip this step.

**If `SEED_FUNCTIONS` is a non-empty JSON array:**

1. Parse the array into individual function names.

2. **Test Convex dev backend reachability** using the first seed function:
```bash
FIRST_SEED=$(echo "$SEED_FUNCTIONS" | node -e "const s=require('fs').readFileSync('/dev/stdin','utf8'); const a=JSON.parse(s); console.log(a[0])")
npx convex run "$FIRST_SEED" --dry-run 2>/dev/null
CONVEX_REACHABLE=$?
```

3. **If connection fails (`CONVEX_REACHABLE != 0`):** Start the Convex dev backend and poll for ready:
```bash
npx convex dev &
CONVEX_PID=$!
...
while [ $ELAPSED -lt 90 ]; do
  npx convex run "$FIRST_SEED" --dry-run 2>/dev/null && break
  sleep 5
  ELAPSED=$((ELAPSED + 5))
done

if [ $ELAPSED -ge 90 ]; then
  echo "⚠ Convex dev backend not reachable after 90s — skipping seed functions"
  # Non-blocking: continue without seeding
fi
```

4. **Run each seed function:**
```bash
for fn in $(echo "$SEED_FUNCTIONS" | node -e "const s=require('fs').readFileSync('/dev/stdin','utf8'); JSON.parse(s).forEach(f => console.log(f))"); do
  if npx convex run "$fn" 2>/dev/null; then
    echo "  ✓ $fn"
  else
    echo "  ✗ $fn (non-blocking)"
  fi
done
```

Seeds are idempotent upserts — safe to run after every plan. Failures are logged but do not block execution.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `D:/Claude/Product Manager/product_master/.claude/get-shit-done/templates/summary.md`.
...
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" state update-progress

# Record execution metrics
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" state record-metric \
...
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" state record-session \
...
```bash
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
