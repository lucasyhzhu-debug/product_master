...
@D:/Claude/Product Manager/product_master/.claude/get-shit-done/references/ui-brand.md
@D:/Claude/Product Manager/product_master/.claude/get-shit-done/references/revision-loop.md
@D:/Claude/Product Manager/product_master/.claude/get-shit-done/references/gate-prompts.md
...
```bash
INIT=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-checker 2>/dev/null)
...
TDD_MODE=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.tdd_mode 2>/dev/null || echo "false")
```

When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@D:/Claude/Product Manager/product_master/.claude/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
```bash
PHASE_INFO=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
AI_PHASE_CFG=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
...
```bash
PHASE_DESC=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
**If found:**
1. Read template: `D:/Claude/Product Manager/product_master/.claude/get-shit-done/templates/VALIDATION.md`
...
SECURITY_ASVS=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```bash
UI_PHASE_CFG=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
```bash
PHASE_SECTION=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
2. Disable Nyquist with the exact command:
   `node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
...
Check config:
```bash
PATTERN_MAPPER_CFG=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.pattern_mapper --default true 2>/dev/null)
```
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @D:/Claude/Product Manager/product_master/.claude/get-shit-done/references/tdd.md to all eligible tasks:
...
- **`## VERIFICATION PASSED`:** Display confirmation, proceed to step 12.5 (plan bounce, if enabled) and step 12.6 (staff review).
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count < 2`:**
    ...
    If "Proceed anyway": accept current plans and continue to step 12.6 (staff review).
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.plan_bounce_passes --default 2)
BOUNCE_SCRIPT=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.plan_bounce_script)
```
...
```bash
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" commit "refactor(${padded_phase}): bounce plans through external refinement" --files "${PHASE_DIR}/*-PLAN.md"
```
...
## 12.6. Staff Review Gate

**Purpose:** Run `/staffreview` on every plan after the checker loop and optional bounce pass. Staff review catches gaps, risks, and misalignments that the automated checker misses (architecture tradeoffs, cross-plan coupling, missing edge cases).

**Skip if:**
- `--gaps` flag (gap-closure plans already have narrow scope — staff review adds noise)
- `--skip-staffreview` flag
- `workflow.staffreview` config is `false`

```bash
STAFFREVIEW_ENABLED=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.staffreview 2>/dev/null || echo "true")
```

If disabled, skip to step 13.
...
◆ Running staff review on phase ${PHASE_NUMBER} plans...
```

Invoke staff review:
```
Skill(skill="staffreview", args="${PHASE_DIR}/*-PLAN.md")
```

**Handle results — ALL findings route to revision, not just Critical.**

staffreview returns tiered feedback:
- **Critical** — blocking issues
- **Important** — significant concerns
- **Refinements** — improvements worth making
- **Minor / Nitpick** — small polish items

Route the COMPLETE tiered list back to the planner — do not filter by tier. Include the full Critical + Important + Refinements + Minor + Nitpick set in the revision prompt. Every finding gets addressed.

**If findings exist:** Re-enter step 12 (revision loop) with the full tiered findings appended to the revision prompt. Cap additional iterations at 2.

**If no findings:** Proceed to step 13.

**Non-blocking error handling:** If the `staffreview` skill fails or is unavailable, log a warning and proceed to step 13. Do not block planning on an unavailable review skill.
...
```bash
node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
if ([[ "$ARGUMENTS" =~ --auto ]] || [[ "$ARGUMENTS" =~ --chain ]]) && [[ "$AUTO_CHAIN" != "true" ]]; then
  node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active true
fi
```
