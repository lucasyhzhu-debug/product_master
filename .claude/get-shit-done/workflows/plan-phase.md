...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@D:/Claude/Product Manager/product_master/.claude/get-shit-done/references/skeleton-template.md`.
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@D:/Claude/Product Manager/product_master/.claude/get-shit-done/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @D:/Claude/Product Manager/product_master/.claude/get-shit-done/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
...
Offer 3 options:
1. **Accept verification** — treat as `## VERIFICATION PASSED` and continue to step 12.6 (staff review)
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
