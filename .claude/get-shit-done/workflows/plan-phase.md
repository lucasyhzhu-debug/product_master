...
## 12.5. Staff Review Gate

**Skip if:** `workflow.triple_review` is `false` in `.planning/config.json` OR `--skip-verify` flag was passed.
...
Spawn staffreview sub-agent with the skill definition:



### Handle Staff Review Return

**ALL findings (Critical + Important + Refinements)** are routed back to the planner for revision:

1. Parse findings from the staff review output
2. If findings exist: spawn gsd-planner with revision prompt including all staff review findings
3. After revision: update success criteria to include staff review items addressed
4. If no findings: display confirmation and proceed to step 13
...
- [ ] Staff review passed (if workflow.triple_review enabled and not --skip-verify)
