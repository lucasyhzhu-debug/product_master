...
If summary shows `DEBUG SESSION COMPLETE`: proceed to Section 5 (Quality Gates).
If summary shows `ABANDONED`: note session saved at `.planning/debug/{slug}.md` for later `/gsd-debug continue {slug}` — skip Sections 5 and 6.

## 5. Quality Gates (After Fix Applied)

**Skip if:** `diagnose_only` is true, session was `ABANDONED`, or fix was "Plan fix" / "Manual fix" (no code changes in this session).

After the session manager returns `DEBUG SESSION COMPLETE` and the fix has been committed:

**5a. Triple review:**

```bash
TRIPLE_REVIEW=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.triple_review 2>/dev/null || echo "false")
```
...
Invoke `Skill(skill="triple-review", args="${slug}")` over the debug fix's diff.
...
If findings exist, spawn gsd-executor to apply fixes and commit as `fix(debug-${slug}): address triple-review findings`.
...
```bash
SIMPLIFY=$(node "D:/Claude/Product Manager/product_master/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.simplify 2>/dev/null || echo "false")
```
...
Invoke `Skill(skill="simplify")`. If proposed changes are applied, commit as `refactor(debug-${slug}): simplify after review`.

**Non-blocking error handling:** Skill failure logs a warning and proceeds.

## 6. Document and Merge (After Quality Gates)

**Skip if:** current branch is `main` (debug ran directly on main) or no fix was applied (Plan fix / Manual fix / ABANDONED).

**6a. Update CHANGELOG.md:**

Read `docs/CHANGELOG.md` (skip with warning if missing). Add entry under `[Unreleased]`:
...
If `gh pr create` fails, surface the error with the branch name so the user can complete the merge manually.

**6c. Squash-merge and sync:**
...
**Non-blocking error handling:** If any post-PR step fails, report the partial state and PR URL so the user can finish the merge manually.
