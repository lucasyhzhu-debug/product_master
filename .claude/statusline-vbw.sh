#!/bin/bash
# GSD Status Line — adapted from VBW 4-line dashboard
...
# Windows: ensure jq is on PATH (WinGet installs here)
export PATH="/c/Users/Irfan/AppData/Local/Microsoft/WinGet/Links:$PATH"
...
# --- Fast cache (5s TTL): GSD state + execution + agents ---
FAST_CF="${_CACHE}-fast"
...
if ! cache_fresh "$FAST_CF" 5; then
  ...
  if [ -f ".planning/STATE.md" ]; then
    PH=$(grep -m1 "^Phase:" .planning/STATE.md | grep -oE '[0-9]+' | head -1)
    TT=$(grep -m1 "^Phase:" .planning/STATE.md | grep -oE '[0-9]+' | tail -1)
  fi
  if [ -f ".planning/config.json" ]; then
    # Auto-migrate: add model_profile if missing
    if ! jq -e '.model_profile' .planning/config.json >/dev/null 2>&1; then
      TMP=$(mktemp)
      jq '. + {model_profile: "quality", model_overrides: {}}' .planning/config.json > "$TMP" && mv "$TMP" .planning/config.json
    fi
    EF=$(jq -r '.effort // "balanced"' .planning/config.json 2>/dev/null)
    MP=$(jq -r '.model_profile // "quality"' .planning/config.json 2>/dev/null)
  fi
  ...
  if [ -d ".planning/phases" ]; then
    PT=$(find .planning/phases -name '*-PLAN.md' 2>/dev/null | wc -l | tr -d ' ')
    PD=$(find .planning/phases -name '*-SUMMARY.md' 2>/dev/null | wc -l | tr -d ' ')
    if [ -n "$PH" ] && [ "$PH" != "0" ]; then
      PDIR=$(find .planning/phases -maxdepth 1 -type d -name "$(printf '%02d' "$PH")-*" 2>/dev/null | head -1)
  ...
  if [ -f ".planning/.execution-state.json" ]; then
    IFS='|' read -r EXEC_STATUS EXEC_WAVE EXEC_TWAVES EXEC_DONE EXEC_TOTAL EXEC_CURRENT <<< \
      ...
      ] | join("|")' .planning/.execution-state.json 2>/dev/null)"
  fi

  # Agent count: works on Linux/macOS with pgrep, graceful fallback on Windows
  AGENT_DATA=""
...
if ! cache_fresh "$SLOW_CF" 60; then
  ...
  if [ -n "$OAUTH_TOKEN" ]; then
    HTTP_CODE=$(curl -s -o /tmp/gsd-usage-body-"${_UID}" -w '%{http_code}' --max-time 3 \
      -H "Authorization: Bearer ${OAUTH_TOKEN}" \
      -H "anthropic-beta: oauth-2025-04-20" \
      "https://api.anthropic.com/api/oauth/usage" 2>/dev/null) || HTTP_CODE="000"
    USAGE_RAW=$(cat /tmp/gsd-usage-body-"${_UID}" 2>/dev/null)
    rm -f /tmp/gsd-usage-body-"${_UID}" 2>/dev/null
  ...
  printf '%s\n' "${FIVE_PCT:-0}|${FIVE_EPOCH:-0}|${WEEK_PCT:-0}|${WEEK_EPOCH:-0}|${SONNET_PCT:--1}|${EXTRA_ENABLED:-0}|${EXTRA_PCT:--1}|${EXTRA_USED_C:-0}|${EXTRA_LIMIT_C:-0}|${FETCH_OK}|" > "$SLOW_CF" 2>/dev/null
fi
...
LEDGER_FILE=".planning/.cost-ledger.json"
if [ -n "$PREV_COST" ] && [ -d ".planning" ]; then
  _to_cents() {
    local val="$1" w f
    w="${val%%.*}"
  ...
  if [ "$DELTA_CENTS" -gt 0 ]; then
    ACTIVE_AGENT="other"
    [ -f ".planning/.active-agent" ] && ACTIVE_AGENT=$(cat .planning/.active-agent 2>/dev/null)
...
if [ "$EXEC_STATUS" = "running" ] && [ "${EXEC_TOTAL:-0}" -gt 0 ] 2>/dev/null; then
  ...
  L1="${C}${B}[GSD]${X} Build: $(progress_bar "$EXEC_PCT" 8) ${EXEC_DONE}/${EXEC_TOTAL} plans"
  ...
  rm -f .planning/.execution-state.json "$FAST_CF" 2>/dev/null
  EXEC_STATUS=""
  L1="${C}${B}[GSD]${X}"
...
L3="$USAGE_LINE"
L4="Model: ${D}${MODEL}${X} ${D}│${X} Cost: ${COST_FMT} ${D}│${X} Time: ${DUR_FMT} (API: ${API_DUR_FMT})"
