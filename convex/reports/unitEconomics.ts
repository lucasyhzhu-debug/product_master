...
// ============================================================================
// Legacy thin wrappers (D-02 zero-downtime bridge; 12 legacy queries preserved)
//
// DEVIATION (Rule 1 - Bug): plan Step 7.1 specifies `snapshot._handler(ctx, args)`,
// but Convex's public type surface does NOT expose `_handler` with its return
// type; calls through `_handler` collapse to `any`, which propagates through
// `useQuery(api.reports.unitEconomics.X)` and breaks existing frontend
// components (npm run build ≈20 TS7006 errors in src/components/analytics/**).
// Wrappers call the exported `*Impl(ctx, args, {})` entry points directly so
// typing is preserved. Deps={} means the module defaults apply — the production
// code path is semantically identical to `._handler`.
// ============================================================================

export const kpiSummary = query({
  args: filterArgs,
  handler: async (ctx, args) => (await kpiAndChannelSnapshotImpl(ctx, args, {})).kpi,
});

export const channelEconomics = query({
  args: filterArgs,
  handler: async (ctx, args) =>
    (await kpiAndChannelSnapshotImpl(ctx, args, {})).channelEconomics,
});

export const channelMomentum = query({
  args: filterArgs,
  handler: async (ctx, args) =>
    (await kpiAndChannelSnapshotImpl(ctx, args, {})).channelMomentum,
});
...
export const byWeekday = query({
  ...
  handler: async (ctx, args) => {
    const { mode, ...filterOnly } = args;
    const snap = await timeSeriesSnapshotImpl(ctx, filterOnly, {});
...
export const rollingTrend = query({
  args: filterArgs,
  handler: async (ctx, args) =>
    (await timeSeriesSnapshotImpl(ctx, args, {})).rollingTrend,
});

export const dayHourHeatmap = query({
  args: filterArgs,
  handler: async (ctx, args) =>
    (await timeSeriesSnapshotImpl(ctx, args, {})).dayHourHeatmap,
});

export const volumeByType = query({
  args: { ...filterArgs, granularity: v.union(v.literal("day"), v.literal("week")) },
  handler: async (ctx, args) => {
    const { granularity, ...filterOnly } = args;
    const snap = await timeSeriesSnapshotImpl(ctx, filterOnly, {});
    return snap.volumeByType[granularity];
  },
});
...
export const typeMixOverTime = query({
  ...
  handler: async (ctx, args) => {
    const { granularity, ...filterOnly } = args;
    const snap = await timeSeriesSnapshotImpl(ctx, filterOnly, {});
    return snap.typeMixOverTime[granularity];
  },
});
...
export const unitsPerTxnByChannel = query({
  args: filterArgs,
  handler: async (ctx, args) => {
    const rows = (await kpiAndChannelSnapshotImpl(ctx, args, {})).channelEconomics;
    return rows
      .map((r) => ({
        channel: r.channel,
        units: r.units,
        orderCount: r.orderCount,
        unitsPerTxn: r.orderCount === 0 ? 0 : r.units / r.orderCount,
      }))
  ...
  handler: async (ctx, args) => {
    ...
    return rows
      .map((r) => ({
        channel: r.channel,
        orderCount: r.orderCount,
        grossAov: r.orderCount === 0 ? 0 : r.gross / r.orderCount,
        netAov: r.orderCount === 0 ? 0 : r.net / r.orderCount,
      }))
...
/**
 * skuPareto legacy wrapper. The snapshot caps at 20 with its own Other row,
 * but the legacy contract lets callers request a custom topN (default 10).
 * Implementing this as a strict wrapper over snap.skuTop would either miss
 * the custom topN cut or return the wrong Other row. Instead we call
 * precomputeBomMaps + loadFilteredData directly (same underlying shared-load
 * path via Convex query dedup) and run reduceSkuTop with the requested N.
 */
export const skuPareto = query({
  args: { ...filterArgs, topN: v.optional(v.number()) },
  handler: async (ctx, args) => {
    const { topN, ...filterOnly } = args;
    const pre = await precomputeBomMaps(ctx);
    const current = await loadFilteredData(ctx, filterOnly, pre.unitsPerProduct);
    return reduceSkuTop(current, pre, topN ?? 10);
  },
});

/**
 * skuChannelMatrix legacy wrapper — same rationale as skuPareto: custom topN
 * means we can't use the capped-20 snapshot directly without losing resolution.
 */
export const skuChannelMatrix = query({
  ...
  handler: async (ctx, args) => {
    ...
    const pre = await precomputeBomMaps(ctx);
    const current = await loadFilteredData(ctx, filterOnly, pre.unitsPerProduct);
    return reduceSkuChannelMatrix(current, pre, topN ?? 8);
  },
});
