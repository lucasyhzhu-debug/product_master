...
| **Subscription credit drawdown in order creation (2026-06-29, PR #216)** | `convex/subscriptions/creditOrder.ts` (new — `createCreditFundedOrder`, `getCreditOrderWhatsappDraft`); `convex/subscriptions/creditReservation.ts` (new — `computeWeekAvailableCredit`); `convex/subscriptions/creditMath.ts` (`computeCreditSplit`); `convex/subscriptions/queries.ts` (`getSubscriptionCreditContext`); `convex/subscriptions/recognition.ts` (draws `subscriptionCreditApplied` at delivery); `convex/subscriptions/outOfCredit.ts` (Path B → reservation model); `convex/whatsappTemplates/render.ts` (new — `renderTemplate`, `SUBSCRIPTION_CREDIT_TOPUP`); `convex/schema.ts` (`orders.subscriptionCreditApplied` additive field) | `src/pages/OrderCreate.tsx` (credit banner wired in — new-order flow only); `src/components/orders/SubscriptionCreditBanner.tsx` (new — presentational); `src/hooks/useSubscriptionCreditContext.ts` (new); `src/components/crm/WeekBackReferences.tsx` (additive Top-up badge) |
