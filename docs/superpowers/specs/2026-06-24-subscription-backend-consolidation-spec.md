...
R1 introduces `recognizeOnDelivery(ctx, orderId, actingUserId?)` in `recognition.ts` as the **single** recognition entry point all 5 sites call. **Hard constraint discovered during planning:** `completeOrder` (`orderCrud.ts`) and `completePackaging` (`packaging.ts`) are plain `mutation()`s whose args are `{ orderId }` only — **they have NO token and NO `ctx.user` in scope**, which is exactly why they call recognition with no author today and rely on the in-function `order.createdByUserId` fallback. Resolving a session user there would require adding a `token` arg — a **breaking contract change** to those mutations and every caller (frontend included). That is out of scope and would NOT be behavior-preserving.

Therefore `actingUserId` is **OPTIONAL**: the 3 status mutations (`statusUpdates.ts:227/536/759`) pass their real acting user (as today); `completeOrder`/`completePackaging` pass `undefined` (as today) → `recognizeSubscriptionDelivery`'s `order.createdByUserId` fallback fires exactly as before. **Net result: R1 is fully behavior-preserving — zero observable change, including ledger author.** The win is narrow and honest: ONE documented recognition entry point (so a future change to the recognition trigger touches one call, not five) + an explicit author parameter at the seam. The seam does NOT absorb the per-mutation edge decision and does NOT duplicate the idempotency / non-subscription guards (those stay inside `recognizeSubscriptionDelivery`, which the seam calls).

**Explicitly rejected:** a heavier `applyForwardTransition` hook that all forward status mutations route through (so `completeOrder`/`completePackaging` stop hand-rolling their status patch). That is real centralization but a much larger blast radius (every forward transition) and would require the token contract change — out of scope for this behavior-preserving slice.

**Acceptance:**
- AC-R1.1 Exactly one function (`recognizeOnDelivery`) is the recognition entry point; all 5 sites call it; `grep recognizeSubscriptionDelivery` shows it called ONLY from inside `recognizeOnDelivery`.
...
- AC-R1.4 **Zero observable change.** The 3 status mutations pass their acting user; `completeOrder`/`completePackaging` pass `undefined` (no token in scope) → `order.createdByUserId` fallback unchanged. Ledger author identical at every site before/after. (No CHANGELOG behavior note needed beyond "internal consolidation.")
- AC-R1.5 Existing recognition tests pass; a new test asserts `recognizeOnDelivery` no-ops for non-subscription orders, is idempotent, and authors correctly both with an explicit `actingUserId` and with `undefined` (fallback path).
