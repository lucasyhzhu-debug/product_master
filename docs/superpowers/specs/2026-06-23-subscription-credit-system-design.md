...
- **Analytics isolation (staffreview I3 → C1, mechanism pinned 2026-06-23):** 1,050 pcs/wk at a confidential B2B price must NOT pollute the existing sales/margin/channel reports. **Pinned mechanism: subscription orders are credit-funded internal fulfilment, NOT channel revenue, and do not enter the financial revenue aggregations at all.** Concretely: if/where internal orders are synced into `externalRevenue`, that sync is gated on `fundingSource !== "subscription_credit"` (and absence of `subscriptionId`), so subscription orders never reach the period-revenue scans. The grounded aggregations to verify clean (none currently filter by source/subscription): `getIncomeStatement` (`convex/reports/incomeStatement.ts:945`), `fetchAndAggregate` (`convex/reports/incomeStatement.ts:581`), `getDailySalesSummary` (`convex/reports/dailySales.ts:8`), `getMultiPeriodPLExport` (`convex/reports/financialExport.ts:283`). **Mandatory success criterion (mechanical sentinel test):** seed a subscription order, run each of the 4 aggregations, assert its qty/Rp is absent from gross/channel totals. **BOM ball-counting (Pitfall #11/#13) MUST still resolve** for subscription products so kitchen/production volume stays correct even though revenue is excluded.
