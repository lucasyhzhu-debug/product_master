...
1. **13:00 prior-day cutoff lock (clause 3) — warn + flag.** A new internal cron `flipDayLocksAtCutoff` (daily **05:25 UTC** = 12:25 WIB, fired *just before* Slice 1's existing 12:30-WIB / 05:30-UTC `change-cutoff` Telegram nudge) sets `plannedDays[].locked = true` on **tomorrow's** day for every active, non-ended week whose cutoff for that day has passed. **Metadata only** — no change to `items`/`lineTotal`/`unitPrice`/credit. UI: `DayPlanCell` surfaces a **non-blocking** "⚠️ past 13:00 cutoff" warning for that day; **editing stays allowed** (true warn+flag).
...
- `flipDayLocksAtCutoff.ts` — `internalMutation` (cron). Iterates active, non-`ended` subscriptions (or their non-terminal weeks), finds **tomorrow's** day in each current/next week, and if that day's change-cutoff has passed (per `changeCutoffHour`/`changeCutoffDayOffset` and `periodRange` WIB math), patches that `plannedDays[]` entry `locked = true`. **Metadata only.** Idempotent (re-flip is a no-op). Bounded scan (cron context, small account count).
- `applyPendingBaselineChanges.ts` — `internalMutation` (cron). For each subscription with `pendingBaselineChange` where `permanentChangeEffective(...) === true` (i.e. `effectiveDate ≤ now`), patches `baselineDailyQty = newQty` and clears `pendingBaselineChange`. Idempotent.
...
- [ ] **AC2 (cutoff lock — clause 3)** `flipDayLocksAtCutoff` sets `plannedDays[].locked = true` on tomorrow's day for every active, non-`ended` week whose cutoff for that day has passed; **lock is metadata only** — no change to `items`/`lineTotal`/`unitPrice`/credit; idempotent (re-run no-op). `saveWeekPlan` still permits editing a locked day (warn+flag, not hard-block).
...
- [ ] **EC2** `flipDayLocksAtCutoff` re-runs (watchdog-free idempotency) → re-flipping an already-`locked` day is a no-op; never un-locks.
...
- **T3 — `flipDayLocksAtCutoff` lock decision (convex-test):** given `now`, `changeCutoffHour`/`changeCutoffDayOffset`, target day → locked or not, via `periodRange` WIB math; idempotent re-run; only tomorrow's day flipped; ended-sub skipped; metadata-only (items unchanged).
...
- **(E-S1)** the `change-cutoff` Telegram nudge at 05:30 UTC — `flipDayLocksAtCutoff` fires at 05:25 UTC so the flag is set before the nudge reads/announces (ordering is best-effort; the nudge in Slice 1 only *notifies*, it does not read the `locked` flag — no hard dependency).
...
- **Q3 (cutoff scan scope — confirm at plan time)** Whether `flipDayLocksAtCutoff` iterates `subscriptions.by_status` (active) → current/next week, or `subscriptionWeeks.by_status` (non-terminal) → sub. Either is bounded in cron context; pick the one that most directly resolves "tomorrow's day" without an unbounded scan (C9). Recommend: iterate active subs, resolve the week containing tomorrow via `by_subscription_weekStart`.
