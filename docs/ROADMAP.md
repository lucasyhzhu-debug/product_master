...
> **Purpose:** Future plans and development progress for Frollie Pro.
...
- [ ] **Partial-credit order payment collection — credit-aware QRIS/bank collection** (follow-up from 2026-06-29 credit drawdown PR #216). For orders created with partial subscription credit (`fundingSource:"deposit"`, `subscriptionCreditApplied` set), the QRIS/bank collection surface (mark-paid, QRIS webhook) should charge `finalTotal − subscriptionCreditApplied`, not `finalTotal`. Full-cover orders (already `paymentStatus:"Paid"`) are unaffected. Scope: `convex/subscriptions/outOfCredit.ts` (Path B), Xendit QRIS charge amount, `markOrderPaid` shortfall guard. Small change.
