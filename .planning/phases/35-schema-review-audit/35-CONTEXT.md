...
Expert audit of all 59 Convex tables to identify data duplication, unused/redundant tables, denormalization waste, missing indexes, and inefficient patterns. Produce a structured audit report and execute safe quick-wins that don't require data migration. No new features, no schema redesigns.
...
- All 59 tables audited with equal depth — no domain weighting. This is a tech debt milestone, thoroughness matters
...
- `feedback` table (UI removed in v1.2): Claude decides — check if table has data and code references, recommend accordingly
- 55 denormalization annotations from v1.0 Phase 8: Verify all annotations against current query patterns — some may be stale after 6 milestones of changes
...
- Phase 8 (v1.0) denormalization annotations — 55 inline comments in `convex/schema.ts` documenting intentional denormalization decisions

### Established Patterns
- Schema uses `defineTable` + `v.object()` validators with explicit field types
- 166 existing indexes across 59 tables (65 `defineTable` calls in 1,600 LOC file)
...
- Known items to investigate: productionCounts (archived?), feedback (UI removed), deprecated fields on menuProducts/orderItems, 55 denormalization annotations freshness
