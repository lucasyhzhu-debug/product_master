...
- Channels with zero data in the selected period are hidden from the chart entirely
...
### Claude's Discretion
- Color palette for 6+ channels (balance brand recognition where possible with visual distinctness — avoid 3 greens clashing)
...
- `ChannelBreakdownCard`: Expandable per-platform revenue rows with outlet drill-down — extend from 3 to dynamic
