...
| 79-01-T2  | 01   | 0    | DA-05..DA-12      | —         | Integration test scaffolding (cron, backfill, invariants, retro, sellThrough, COGS) | integration | `npm run test -- --run convex/integrations/bigseller/__tests__/cron.test.ts convex/bigsellerOrders/__tests__/backfill.test.ts convex/externalData/__tests__/revenue-invariants.test.ts convex/externalData/__tests__/retroactive-mapping-shopee.test.ts convex/externalData/__tests__/sell-through-shopee.test.ts convex/reports/__tests__/incomeStatement-shopee.test.ts` | pending Wave 0 | ⬜ pending |
...
| 79-03-T1  | 03   | 2    | DA-05, DA-11      | T-79-01, T-79-02, T-79-03 | No cross-platform leak; DA-11 deferral documented | integration | `npm run test -- --run convex/externalData/__tests__/revenue-invariants.test.ts convex/integrations/bigseller/__tests__/helpers.test.ts` | ✅ (Wave 0) | ⬜ pending |
...
| 79-07-T2  | 07   | 3    | DA-10             | —         | UI toasts + buttons                                        | component   | `npm run test -- --run src/components/salesAnalytics/__tests__/BigSellerSyncPanel.test.tsx` (create if absent) | ❌ manual | ⬜ pending |
| 79-07-T3  | 07   | 3    | DA-11, DA-13      | T-79-15   | 24h label; no buyer columns (deferral)                     | component   | `npm run test -- --run src/components/salesAnalytics/__tests__/BigSellerOrdersTable.test.tsx` | ❌ manual | ⬜ pending |
...
*Plan 01 Task 3 flips `wave_0_complete: true` and checks all boxes once the 9 files land.*
...
- [x] Wave 0 covers all MISSING references (9 files × 9 requirements)
