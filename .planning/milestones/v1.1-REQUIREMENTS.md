# Requirements Archive: v1.1 Stabilization & QoL
...
### Kitchen Infrastructure (deferred)
- **KIF-01**: Multi-depot GoFood kitchen flow — support N depots (not just Goldfinch), kitchen packing UI shows depot per order/batch, per-depot stock tracking. Crystal outlet coming online soon. Affects PackingPanel, KitchenViewV2, k3martKitchen queries.

### Order UX Polish (deferred)
- **OUX-01**: Expedite warning on Today/Tomorrow date pills — show inline text "This order will be auto-expedited when payment is received" when selecting near-term due dates
...
- **TDT-02**: E2E Playwright test suite — current coverage is unit tests only (184 tests), no end-to-end browser tests
- **TDT-03**: Apply generic query factory (`listAll`, `getById`, `textSearch`) to remaining ~26 query files — Phase 5 covered 5 simple entities only
- **TDT-04**: Update `src/lib/types.ts` to use Convex-generated types instead of manual TypeScript interfaces
- **TDT-05**: Remove `useConvex` prefix from hook filenames (cosmetic, low priority)

### Infrastructure (deferred)
- **INF-01**: React error boundaries — catch and display component-level errors gracefully instead of white screen
- **INF-02**: CSV/Excel export for reports — sales analytics, inventory, orders exportable to spreadsheet

### Enhanced UX (deferred)
- **EUX-01**: Order templates for repeat customers — save and reuse common order configurations
...
*Last updated: 2026-02-16 — added 10 missing v1.2 deferred items from scan (KIF-01, OUX-01, TDT-02-05, INF-01-02, EUX-01)*
